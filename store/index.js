...
export const actions = {
  ...
  async fetchHomePage ({ commit }) {
    const homepage = await axios.get('https://www.jbiddulph.com/api/pages/home')
    commit('setHomePage', homepage.data)
    return homepage.data
  },
  // GET PAGES
  async fetchPages ({ commit }) {
    const pages = await axios.get(`https://www.jbiddulph.com/api/pages/${this.params.slug}`)
  ...
  async fetchLocationHeaders ({ commit }, location) {
    const headers = await axios.get(`https://www.jbiddulph.com/api/locations/${location}/headers`)
